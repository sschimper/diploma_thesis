...
\begin{figure}[!tbp]
	...
	\hfill
	\subfloat[Constructive solid geometry: a grooved sphere.]{\includegraphics[width=.3\textwidth]{img/3 approach/csg_shape.png}\label{fig:art_csg}}
...
Therefore we add a reference to the \texttt{Combined Attributes} object of a shape to the struct that is defining the node for the intersection collection linked list. This struct is shown in Listing \ref{lst:linked_list_node}.


\begin{listing} 
	\begin{lstlisting}[caption={Representation of a node of the linked list, in which the collected intersections are stored, together with a reference to the \texttt{Combined Attributes} node of the associated shape.}, label={lst:linked_list_node}]
...
\subsection{Approach 2: Initializing CSG tree root as user defined geometry}
...
\subsection{Approach 3: Construction of individual KD trees for CSG}
The folloi


\subsection{Final realization}
