...
The following subsections are based on the lecture notes of the course \citetitle{cg3} held at Charles University in Prague, Czech Republic \cite{cg3}.
...
Multiple approaches for solving the rendering equation exists, for example the \emph{radiosity} method, aiming at solving it via the application of the finite element method (compare \cite{goral1984modeling}), or \emph{distributed ray tracing}, a technique for achieving effects such soft shadows by distributing cast rays "according to the analytic function they sample" \cite{cook1984distributed}. \todo{weird sentence} However, the most commonly used methods for solving the rendering equation are Monte Carlo methods. \todo{why??} One particular Monte Carlo method is \emph{path tracing}, which is the subject of this section.
...
\begin{figure}
	...
	\caption{Figure from the paper \citetitle{kajiya1986rendering} by Kajiya \cite{kajiya1986rendering}.}
	\label{fig:kajiya_figure}
\end{figure}

With this knowledge we can re-write Equation \ref{eq:renderingeq} as follows:
...
\todo{variance and converging stuff}
...
Embree supports a variety of geometric shapes such as triangles, quads, and all sorts of curves \todo{rephrase}.
